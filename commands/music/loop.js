    ...
    execute(client, message, args) {
        ...
        if (args.join(" ").toLowerCase() === 'queue') {
            if (client.player.getQueue(message).loopMode) {
                ...
                return message.channel.send(`${client.emotes.success} - Repeat mode **off** !`);
            } else {
                client.player.setLoopMode(message, true);
                return message.channel.send(`${client.emotes.success} - Repeat mode **on** queue bakal loop terussss !`);
            };
        } else {
            if (client.player.getQueue(message).repeatMode) {
                ...
                return message.channel.send(`${client.emotes.success} - Repeat mode **on** musicc sekarang bakalan di loop terusss !`);
            };
        };
    },
};